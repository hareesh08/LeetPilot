 ...
 * Note: This provides basic obfuscation. For production use, consider more robust encryption.
 */
class EncryptionUtils {
   ...
   */
  static generateKey() {
    const extensionId = chrome.runtime.id || 'leetpilot-default';
    const timestamp = Date.now().toString();
    return btoa(extensionId + timestamp).slice(0, 16);
  }
...
class StorageManager {
  ...
  async initializeEncryption() {
    try {
      ...
      this.encryptionKey = 'leetpilot-default';
    }
  }
  ...
  async storeAPIKey(config) {
    try {
      ...
      // Encrypt the API key
      const encryptedApiKey = EncryptionUtils.encrypt(config.apiKey, this.encryptionKey);
  ...
  async retrieveConfiguration() {
    try {
      ...
      // Decrypt the API key
      const decryptedApiKey = EncryptionUtils.decrypt(config.encryptedApiKey, this.encryptionKey);
