...
class ComprehensiveErrorHandler {
  ...
  cleanupOldRetryAttempts() {
    // Remove retry attempts older than 10 minutes
    const cutoff = Date.now() - 10 * 60 * 1000;
    
    for (const [requestId, attempt] of this.retryAttempts.entries()) {
      // If we don't have timestamp info, remove old entries based on size
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  globalThis.LeetPilotErrorHandler = { ComprehensiveErrorHandler };
} else if (typeof self !== 'undefined') {
  // Web worker context - attach to self
  self.LeetPilotErrorHandler = { ComprehensiveErrorHandler };
}