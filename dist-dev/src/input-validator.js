...
class InputValidator {
  constructor() {
    ...
    this.allowedProviders = ['openai', 'anthropic', 'gemini', 'custom'];
  }
  ...
  validateConfiguration(config) {
    ...
    if (config.provider === 'custom') {
      if (!config.customApiUrl) {
        result.errors.push('Custom API URL is required for custom providers');
      } else {
        const urlValidation = this.validateCustomApiUrl(config.customApiUrl);
        if (!urlValidation.valid) {
          result.errors.push(...urlValidation.errors);
        } else {
          result.sanitized.customApiUrl = urlValidation.sanitized;
        }
      }
    }
